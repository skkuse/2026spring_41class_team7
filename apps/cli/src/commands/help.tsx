...
export default function Help({ showLogo }: { showLogo?: boolean }) {
  return (
    <Box flexDirection="column">
      ...
      <Text>jobclaw evaluate [path] — Repo scan → scan-result.json, then backend rubric (OpenAPI, Zod, …)</Text>
      <Text dimColor>  env: OPENAI_API_KEY · --type · --model · --json · --out FILE.md|json</Text>
      <Text>jobclaw publish [path] — Publish scan + latest evaluation (requires both under .jobclaw/)</Text>
