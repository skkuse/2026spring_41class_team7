...
import { bundleBackendEvaluationContext } from "./bundle-backend-context.js";
import { getOpenAiApiKey } from "./credentials.js";

const PROMPT_REL = "../../prompts/evaluations/node-backend-evaluation.prompt.md";

export type BackendEvaluationCriterion =
  | "openapi"
  | "zodValidation"
  | "rateLimiting"
  | "caching"
...
export type BackendEvaluationScores = Record<BackendEvaluationCriterion, number>;
...
export type BackendEvaluationResult = {
...
export async function loadEvaluationPromptTemplate(): Promise<string> {
...
function normalizeResult(raw: unknown): BackendEvaluationResult {
  const o = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
  const sc = o.scores && typeof o.scores === "object" ? (o.scores as Record<string, unknown>) : {};
  const scores: BackendEvaluationScores = {
...
export type RunBackendEvaluationOptions = {
  repoRoot: string;
  model?: string;
};

export async function runBackendEvaluation(
  options: RunBackendEvaluationOptions,
): Promise<{
  ...
  const template = await loadEvaluationPromptTemplate();
  const root = path.resolve(options.repoRoot);
  const rubric = template.replace(/\{\{ROOT\}\}/g, root);

  const context = await bundleBackendEvaluationContext(root);
  const model =
    options.model ??
...
The repository files below are truncated excerpts from disk at ${root}. Evaluate based only on this evidence.
