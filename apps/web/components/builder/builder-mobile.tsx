            ...
            return (
              <div key={id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
                <div className="w-5 shrink-0">
                  {isDone && <Icon icon="solar:check-circle-bold" className="text-base text-emerald-500" />}
                  {isActive && <Icon icon="solar:spinner-bold" className="animate-spin text-base text-primary" />}
      ...
      <header className="sticky top-0 z-10 border-b border-border bg-background/90 px-4 py-3 backdrop-blur">
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onSave} disabled={!allDone}
            className="rounded-lg bg-primary px-3 py-1.5 font-mono text-[9px] font-black uppercase text-primary-foreground disabled:opacity-40">
            Save
          </button>
        </div>
      </header>
